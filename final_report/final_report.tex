...
The network of Relay Agents implementing the Framework are colloquially referred to as ``The Deep End'' (as opposed to the typical application terminology of ``Front End'' and ``Back End'').
...
In order to define \emph{how} the visible graph changes over time (as a result of behaviours switching) we now introduce the concept of ``Behaviour Transition'' in equation~\eqref{eq:def:btg}, whose edges are the incurred time-cost associated with a given behaviours transition (i.e. ``Yellow''-time between ``Green'' and ``Red'' states) as defined in equation~\eqref{eq:def:btg_costs}.
...
The first step in performing this estimation, is determining which portion of the downstream signal is relevant â€“ meaning what part of the signal should be considered for the estimation. 
...
For a given behaviour, some edge probabilities will always be 0 â€“ these edges are not allowed paths â€“ and therefore these could be removed and the problem space reduced. 
This method was determined to be infeasible due to the fact that a large set of optimization problem would need to be defined, one for each possible behaviour â€“ the Python implementation for optimization requires that constraints be explicitly written for the procedure to work. 
...
An example could be limiting the amount any edge probability can change in one cycle and penalizing ones that attempt to change by larger than this threshold â€“ as this is likely an anomaly or incorrect identification of downstream signals.
...
This can be tuned, on an agent-by-agent basis, to alter how quickly these values are modified and adapt â€“ an average of the two values is a good starting point, to enable quick transitions, while remaining relatively stable. 
...
This design choice allows for each agent to ``talkÓ with its neighbors, gaining access to all information stored. 
This ``sharingÓ of information is essential to keep agents informed on events occurring in the neighborhood.
Information is either requested or directly sent to a specific agent, depending on the type: current queues, predictions, plans, events, etc.
 Agents can freely communicate by simply ``askingÓ their neighbors for the specified data. 
...
This information also creates useful visualizations for traffic engineers, giving them the ability to ``see into the futureÓ. 
While they wouldnÕt necessarily act upon this information, they would have an expectation of traffic at each intersection. 
It also gives context for why the next behaviour was selected (i.e. the ``plan") Ð this is discussed further in later sections.
...
This is performed in the same manner as a first-order prediction, but utilizes each neighbour'€™s current incoming prediction signal as well, $Pr_{k,i}^{nbr}$.
 If even larger predictions are required, predictions can be extended to more distant nodes, but these would be very low quality and carry low weight â€“ these are multiplied by each intersections edge probability, intrinsically lowering their strength. 
...
As discussed in the engineering approach for ``Learning in Relay''€ a sophisticated solution for behaviour edge probability learning was not found.
 Further research and development would need to be conducted to derive a solution to his problem â€“ the system is underdetermined, and either requires a reduction of design parameters or other reformulation to become solvable. 

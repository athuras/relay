...
\item[DOJO] A client-side JavaScript framework.
...
\item[Erlang] A functional concurrent programming language used to build massively scalable soft real-time systems. Some of its most prevalent uses involve high-performance telephony, and instant-messaging infrastructure.
\item[Info Box (Map)] Info Box is a term used to describe a popup-style interface widget which contains information about an element on the display. In the context of a map, info boxes are used to present information about a location by displaying an overlay on the map in close proximity to the location of interest.
\item[OTP] The Open Telecom Platform, a set of Erlang libraries designed to be used to create robust distributed systems.
...
\item[MV] Another software development pattern. MV is similar to MVC, however there is no controller, as controller functionality is built into the view class. This pattern is common in less complex solutions or in languages which do not support a rigid class-based system.
...
The deep-end simultaneously refers to the design of the underlying traffic control agents, as well as the network of connected agents as a whole.
...
Fundamentally, there should be no functional or developmental distinction between a single Agent operating in isolation, and a large network of sparsely connected agents.
...
Agent design should be able to characterize how traffic would be expected to flow through the intersection it controls, including any probabilistic variance associated with that expectation.
...
Agent design should facilitate a ``plug-and-play'' optimization routine based on the underlying mathematical representations of the local environment.
In particular, the scope of the optimization routines should be limited to the underlying state (and predicted state) of the agent.
This encapsulation of \emph{state} should provide sufficient transparency to model arbitrarily complex queue/accumulator-based models.

\subsubsection{Communication}
In order to reduce the network traffic, and provide impetus for the communication of informationally \emph{dense} messages between agents; the Agent design to expressly prohibit arbitrary network communication between agents, providing instead the concept of a ``neighbourhood'' which can be parametrized to optimize communication under bandwidth limitations.

\subsection{Functional Requirements for Agent Implementation}

The extent to which the prototype Agent implementation meets the high-level design/architectural goals is summarized in the following table, specifically, the binary pass/fail of a particular test can be considered a functional \emph{unit}, (and thus sufficient fodder for a comprehensive suite of unit tests.

\begin{longtable}[H]{| P{4cm} | P{10cm} |} 
\hline
    Test                                & Description                                                                                               \\ \hline
    [network] Request data from peer    & The agent of interest can request information from its peers, and then take action based on the response.  \\ \hline
    ...
    [predict] Request new Prediction    & Can the agent request (and receive a response) from the prediction service?                               \\ \hline
...
To help us tackle traffic control itself, we assumed that we have perfect (albeit significantly limited in scope) traffic data and built a system on top of this.
...
To reduce complexity for this project, only intersection with 4 directions are being modelled. 
...
Angular.js is the smallest candidate framework at 36kb. Angular.js also has no prerequisite JavaScript libraries, and is thus completely independent. Although Angular.js offers rich DOM and interaction functionality, it provides few utility features for data structures and architectures. Angular.js does not follow a rigid MVC or MV structure, which was a major downfall for the framework. Angular.js does not require much overhead code to implement, primarily because it does not follow a rigid software architecture, and thus is more functionally-oriented.
...
Various technical challenges were encountered throughout the development of the Relay Application. These issues related predominantly to an incomplete understanding of core selected JavaScript frameworks and libraries used in the solution.
...
The database contains information for: road metrics, intersection metrics, events, and Toronto's intersections and roads, acquired from their public data website.

'Road metrics' stores the time delay kernel parameters for each segment, along with a timestamp and associated node ids at the ends of the segment. 
...
The network of Relay Agents implementing the Framework are colloquially referred to as ``The Deep End'' (in constrast to the typical application terminology of ``Front End'' and ``Back End'').
...
At the highest level, the loosely coordinated (read: not mesh) network of Relay \emph{Agents} tackles the traffic control problem through the intelligent use of local computation, rather than a single large-scale optimization problem to be undertaken sparsely at a centralized facility.
...
In concert, the Network achieves a sort of pseudo-temporal consensus, which itself acts as a stabilizing factor on the decisions made by each agent.
In principle, the predictive consensus that is possible within the Relay Framework makes it \emph{easier} to achieve timing synchronization between peers, despite not being a tightly coupled system.

We hypothesize that the added flexibility associated with such a scheme (i.e. being robust to failures, and reacting to anomalies) outweighs the potential cost associated with not having a centralized service.
...
Rationale for pursuing a sparsely connected peer-to-peer topology is presented, along with our approach for modelling uncertainty.
...
The concept of a generalized intersection within the Relay Framework is formalized in Section~\ref{sec:def:agent}.
This view of the intersection can be interpreted as an instantaneous probabilistic transfer function: given that a vehicle enters an inlet, the emergent edges from that inlet represent the probability that the vehicle will \emph{emerge} at the prescribed outlet.
...
The advantages associated with a geographically distributed system are revolve around the reduction in concentration risk---for example, given a regional power-outage, or natural disaster the liklihood that the entire network is affected is low if it is distributed across a large region.
This of course comes at the cost of added complexity in agent design (i.e. in a typical peer-to-peer sense, each Agent becomes simultaneously a client and server), as well as increasing the relative scarcity of inter-agent communication bandwidth.
...
This design breaks with the traditional centralized (or recently, ``Cloud Based'') approach to traffic synchronization, which is used extensively by modern installations (i.e. the ranks of which include Miovision's Spectrum).\\
...
As shown in Figure~\ref{fig:Relay_Network}, each Agent is only connected to its immediate peers (with supplementary connections made available on-demand---such as for system upgrades etc.).
Thus the network that each agent \emph{sees} looks like a full mesh centred around it.
...
As alluded to above, the network connectivity restrictions placed on the Relay network imply a severely reduced view of the traffic environment as a whole.
...
In isolation, each Agent in the \emph{expected} path adjusts their behaviour schedules to align with the expectation.
Once the adjustment calculation is complete (which in an ideal world is instantaneously), the Agent then propagates a modified wave to its peers, and so on.

\paragraph{Leveraging Asynchronous Distributed Computation}
In an abstract convergent sense, the network aligns itself to the most likely routes that this hypothetical vehicle would take.
In essence, we \emph{cheat} the connectivity problem, by infusing each message with as much computation as is reasonable (after all, prediction is in general computationally expensive), this acts as a form of compression, albeit in terms of \emph{work}, the actual messages sent `over the wire' could also be compressed as well---but thats an implementation problem.

If we take a snapshot of an Agent in time, we would expect to find a large amount of information that represents not just the local environment, but something that reflects the superposition of many peer's views of the network.
...
In Relay, there are two principal probabilistic models, the first is the intersection transition graph (as described in Section~\ref{subsubsec:intersection_modelling}, and formalized in Section~\ref{sec:def:intersection_graph}), which models the liklihood that a vehicle will take a particular action at an intersection.
...
The second of which is the delay kernel, which is ultimately a statistically learned parameter which is formalized in Section~\ref{sec:def:expected_ingress}, in the context of informing predictions.
...
At its heart, Relay is a \emph{dynamic graph automata}, whose state changes (the most visible of which is the underlying traffic light values) are based on a combination of inputs from the environment, and its immediate neighbours in the Relay network.
Each traffic controller in Relay is an autonomous \emph{Agent}, that (as the network deepens/expands) forms transient, asynchronous communication channels to its neighbours.
...
$G$ has some notable properties that allow for some algebraic simplifications, the most significant of which is that there can only be one edge connecting node $i$ to node $j$, and that we can treat it as a directed adjacency graph, making it trivial to express it efficiently as a matrix.
...
Behaviours in Relay control the \emph{visible} intersection transition graph of the intersection, by operating on the values of particular edges.
...
Formally, behaviours in relay are element-wise masks on $\bf{G}$, each of which is a member of the finite set $\bf{B}$ as shown in equation~\eqref{eq:def:behaviours}.

\begin{equation}\label{eq:def:behaviours}
...
Ultimately, the most important property of the Behaviour, is that in composition with $\bf{G}$ it forms what we'll hereafter refer to the ``visible'' intersection graph, or rather the physically realizable transfer graph of the intersection.
...
\end{equation}
where $B_k$ is the $k^{th}$-member of the set of behaviours $\bf{B}$, and ``$X \circ Y$'' is the element-wise/Hadamard product of matrices $X$ and $Y$ (which must have the same shape).
...
\end{equation}
where $A_i$ (in the idealized representation) is the set of ingress events $\{(\omega, \tau)\ : \tau \in [-\infty, t]\}$, or rather, the set of all ingress events that have been observed at the $i^{th}$-node, and $\delta(\cdot)$ is the Dirac Delta function.
...
\subsubsection{Behaviour Transition}
\label{sec:btg}
...
In equation~\eqref{eq:def:behaviours}, we referred to a set of behaviours $\bf{B}$, and in equation~\eqref{eq:def:egress} we referred to the time-varying graph $V(t)$.
In order to define \emph{how} the visible graph changes over time (as a result of behaviours switching) we now introduce the concept of the  ``Behaviour Transition Graph'' as defined in equation~\eqref{eq:def:btg}, whose nodes are members of $\bf{B}$ and whose edges are the incurred time-cost associated with a given inter-behaviour transition (i.e. ``Yellow''-time between ``Green'' and ``Red'' states) as defined in equation~\eqref{eq:def:btg_costs}.
...
Within the Relay implementation we have frequently used time-values corresponding to the legally mandated ``yellow'' time associated with transitions to ``Red'' (on the order of 2-seconds), or the dead-time associated with transitions from the ``Red'' behaviours.

\begin{equation}\label{eq:def:btg}
	\bf{\Gamma} = G(\bf{B}, \Tau)
\end{equation}

Thus we define the Behaviour Transition Graph $\Gamma$, as a directed graph whose nodes are behaviours (members of $\bf{B}$) and whose edges are the associated costs for transitions between adjacent behaviours.
...
For example, if we know that on average the train will take 20-minutes to travel from the neighbouring train station to the local one, we can  generate an expected arrival time for the train.
...
\end{equation}
where $e_j^{nbr}(t)$ is the egress signal (as defined in equation~\eqref{eq:def:egress}) of the neighbouring agent that is upstream of the $i^{th}$ inlet of the local agent, and $E\left[ \cdot \right]$ denotes the expected value.
...
In Section~\ref{sec:def:expected_ingress} we formulated the expected ingress signal in terms of a delay kernel $h(\tau)$ and a superposition of signals from a neighbouring agent---explicitly relying on inter-agent communication, and prior knowledge of the particular delay distribution between the connected agents.

In a similar fashion, the expectation of the local egress signal follows a similar flow, albeit with a subtle variation.
In equation~\eqref{eq:def:expected_ingress} the expected ingress was formed (in part) from a signal derived exclusively from real measurements; namely $e^{nbr}_j(t)$---a neighbour's egress signal, which via equation~\eqref{eq:def:egress} was directly derived from real measurements (incidence signals).
...
\begin{align}\label{eq:def:expected_egress}
	E\left[e_j\right](t) &= \sum_{i=1}^n E\left[a_i(t) V_{ij}\right(t)]\\
						&= \sum_{i=1}^n E\left[a_i\right](t) E\left[V_{ij}\right](t) + \mbox{Cov}(a_i, V_{ij})(t)\\
...
Due to the co-recursive definition of the expected egress and ingress, the results from one calculation can propagate through a closed loop of peers, thus becoming correlated to the other.
...
However, it may be the case that in the \emph{real} world, it is unlikely that individual participants would exhibit geographically cyclic behaviours exaggerated enough to damage the validity of the expected egress prediction (i.e.\ driving in circles).
To add even greater ambiguity, we hypothesize that the introduction of bias due to the assumption of statistical independence may actually increase performance in tightly coupled networks (i.e.\ by implicitly biasing agent's timing in favour of things with cyclic behaviours, such as public transport and logistic vehicles).

\subsubsection{Acyclic Scheduling}
\label{seq:def:plan}

In a sense, the final piece of the Relay Agent puzzle; the Agent internally stores a \emph{plan} that governs when, and to which behaviours the agent will transition over some future time window.
The ``Behaviour Transition Plan'' (BTP, or simply ``plan'') is the combination of a particular \emph{path} through the behaviours transition graph (defined in equation~\eqref{eq:def:btg}) with the times associated with each step in the path.
Given a plan, it is thus possible to generate the visible graph signal $\bf{V}(t)$, and via a behaviours lookup-table, this process is invertible.
...
Which is to say, that each successive step in the plan must take place after the current step plus the cost of taking the step (as formalized in equation~\eqref{eq:def:btg}), and that each successive behaviours, must be adjacent-reachable in $\Gamma$.
...
In equation~\eqref{eq:def:behaviour_signal} we define how the plan defines the behaviour signal.
...
\begin{equation}\label{eq:def:behaviour_signal}
...
\begin{equation}\label{eq:def:u}
	u(\tau) = \left\{
		...
		\right.
	\end{equation}
It is through manipulations of $\Gamma$ that the agent is able to consider various behaviours, as well as limit the acyclic behaviours associated with a \emph{plan} to legal states (for both real-world and abstract definitions of ``Legal'').

%%  LEARNING IN RELAY  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Learning in Relay}
\label{sec:learning_in_relay}

Each agent within the system has multiple design parameters that will constantly update. With each cycle---or with each behaviour transition,  predictions and estimations are made to recalculate these parameters. 
...
However, this is not a fixed time, so the delay is modelled using a gamma distribution.
The choice of gamma, represents a first-pass approximation, but offers some useful properties.
For example: the distribution can be parametrized to have a hard lower-bound, meaning we don't have to worry about vehicles travelling \emph{faster} than some arbitrary speed limit. 
...
\tau_{j} & time delay distribution along edge connecting agent node $j$ to neighbour \\
\end{conditions*}
...
Where:
\begin{conditions*}
D & vector of neighbour departing signals \\
C & vector of arrival signals \\
\tau & vector of time delay estimations
\end{conditions*}
...
Higher-order predictions, ones that require information from the larger neighbourhood, can also be generated, but with less fidelity. 
The details on how these predictions are performed is discussed below.

\subsubsection{Utilizing Neighborhood Information}
As discussed in Section~\ref{sec:p2p_philosophy}, agents are connected to each of their neighbors, forming a grid-like mesh structure. 
...
An event centre is constructed on each agent to handle incoming and outgoing requests, making communication efficient and standardized across the network (discussed further in later sections).
	...
	While an agent is not directly connected to other agents in its extended neighbourhood, if required, it can indirectly access this information. 
Any agent can ``inject" requests into its neighbours to obtain information from their neighbours, and so on (again, specifics discussed in later sections). 
This is not used often, as fidelity decreases quickly as requests propagate outward, but, for example, it is necessary for calculating higher-order predictions.


\subsubsection{Making a Prediction}
To make a prediction, the agent must combine the learned edge probabilities with the time delay estimates for each neighbouring agent. 
This utilizes previously calculated information about the agents in the neighbourhood, to derive a predicted incoming signal for each node at the agent of interest. 
...
If the length requested is longer than this next-neighbour prediction, a higher-order prediction is calculated, known as $Pr_{i}^{*}$.
...
Where:
\begin{conditions*}
p_{kj}^{nbr} & edge probability for signal at $k^{th}$ neighbour node, exiting $j^{th}$ node \\
d_{e_i} & expected departing signal from neigbhour node connected to agent node $i$ \\
c_{k,i}^{nbr} & real incoming signal at $k^{th}$ node of neighbour agent connected to agent node $i$ \\
\tau_{i} & time delay distribution along edge connecting agent node $i$ to neighbour \\
k & neighbour inlet id \\
j & neighbour outlet id
\end{conditions*}


\subsubsection{Higher-Order Predictions}
As mentioned previously, if the requested prediction is longer than the neighbours can provide, a higher-order, or remote, prediction is made. 
This is performed in the same manner as a first-order prediction, but utilizes each neighbour'��s current incoming prediction signal as well, $Pr_{k,i}^{nbr}$.
...
Where:
\begin{conditions*}
Pr_{k,i}^{nbr} & prediction signal at $k^{th}$ neighbour node, connected to agent node $i$ \\
p_{kj}^{nbr} & edge probability for signal at $k^{th}$ neighbour node, exiting $j^{th} $node \\
d_{e_i} & expected departing signal from neigbhour node connected to agent node $i$ \\
c_{k,i}^{nbr} & real incoming signal at $k^{th}$ node of neighbour agent connected to agent node $i$ \\
\tau_{i} & time delay estimation along edge connecting agent node $i$ to neighbour \\
k & neighbour inlet id \\
j & neighbour outlet id
\end{conditions*}
...
We then described how the behaviour signal was defined in terms of the plan in equation~\eqref{eq:def:behaviour_signal}.
...
Some examples we have used successfully are outlined in the following sub-sections.
...
\end{equation}
Where $\eta_i$ is the associated congestion \emph{rate}, and $r_i$ as before is the un-congested rate.
...
\end{equation}
Noting that $\beta_i$ is defined in terms of the agent's visible graph as of equation~\eqref{eq:def:gating_beta}, which is in turn defined based on the behaviour signal.
Thus all other factors remaining constant, the inlet flux $\Phi_i$ is the result of an explicit function of the behaviour chosen, and thus the underlying path.

Ultimately, the total biased-flow through the intersection, as a function of the plan $p$ which parametrizes $B$ by~\eqref{eq:def:behaviour_signal}, which in turn defines $\beta$ via equation~\eqref{eq:def:gating_beta}.
...
Subject to:
\begin{align}
	d &\leq d_{max} \in \mathbb{Z}\\
	t_d &\leq t_{max} \in \mathbb{R}
\end{align}
recalling that $d$ is the depth of the path $p$, and that $t_d$ is the last time associated with the path.
Further recall that all constraints placed on the validity of a particular path are encapsulated in the definition of the path $p$, (i.e. they must be temporally successive, and infinitesimally short steps are prohibited via the Behaviour Transition Graph).

Note this is a combination set-cover (NP-complete), and nonlinear programming problem.
For efficiency, it is best to separate the integer aspect out (separate out the time component of the path from the graph component), this separation is a classic dynamic programming technique, and is described below:
\begin{equation}\label{eq:def:minlp_objective}
	\argmax_{p.B} \argmax_{p.t} \bf{\Phi}_{p.B}
\end{equation}
where $p.B$ is the sequence of behaviours (irrespective of their relative timing), and $p.t$ is the timing (given a behaviour sequence $p.B$.
Thus for each behaviour, we evaluate the best possible flux given changes to the relative timing, and use the combination $p =  (p.B, p.t)$ that gives the best outcome.
...
The prototype implementation was constructed using a combination of the Erlang and Python programming languages, in conjunction with the Erlang Cowboy WebServer (for exposing a REST API to the back-end), and ErlPort: a python interface to the Erlang Port protocol, that enables (relatively) easy interoperability between disparate Python and Erlang processes.

A high-level overview of the implementation is detailed in Figure~\ref{fig:agent_implementation}.
\begin{figure}[H]\label{fig:agent_implementation}
...
Rather than being explicitly controlled by the Controller, the controller simply periodically requests that they perform their function, and independently updating the State Manager when their computation is complete (some operations can take a long time).

The Python Port exposes the Erlang Application to one-or-many stateless Python processes, which perform the necessary floating point work via NumPy routines, everything else in the system is written entirely in Erlang. The Controller is described in the figure below.

\begin{figure}[H]\label{fig:agent_controller}
	\includegraphics[width=\textwidth]{figures/Relay_Agent_Controller.png}
	\caption{The Relay Agent Controller}
\end{figure}
...
If concurrent operation, scalability and fault-tolerance are Erlang's strengths, floating point work (colloquially ``Number Crunching'') is one of its most predominant weakness (in the context of implementing Relay).
...
These tests are centred on the Flask server, and cover information and request handling from the Interface, Framework, and database. 
...
\begin{longtable}[H] {| P{3.5cm} | P{10cm} | P{1.5cm} |}
    ...
    Return data to Interface         &  Server successfully returns requested information to Interface. Data is parsible and useable by Interface. & Pass    \\ \hline
...
These tests are centred on the components of the Framework, and cover information and request handling from the Flask server, between Erlang processes, Python computational components, as well as inter-agent communication. 
The table below contains each test and results from testing on the final prototype.
...
Thus the most salient failure of our prototypical implementation is the lack of conclusive results.

The Agents could communicate, and generate signals, however the underlying optimization routine was being stimulated with \emph{fake} data, and it could be argued that the network had insufficient depth for us to truly define whether significant optimization was taking place.
...
A common adage within the traffic engineering community to this point is that ``Adaptive Systems are great, but in gridlock, the best system will be equalled by a stop sign''.
...
\subsection{Exploiting Neighbourhood Redundancy - The ``Super Node"}  % Possibly redundant.
...
Another stepping stone could be the introduction of another layer of consensus, one that explicitly outlines behaviours such as dynamic route biasing, discrete platoon management etc.
...
It was found that deadlines were often delayed between major milestones, such as reports, presentations and demonstrations. This often lead to the compression of work into short periods of time leading up to the project. This was expected given the nature of the time frame for the team members, however further steps should be taken to minimize the impact of this in the future.

Throughout both terms, the team struggled to meet on a regular basis. This had a negative impact on the team's adherence to deadlines, communication, and ability to effectively delegate work. In the future, consistent meetings should be enforced to ensure the team's adherence to the project plan.
...
The Relay Application exposes rich information which was previously either impossible or extremely expensive to collect, greatly reducing the costs associated with maintaining the efficiency of the transportation network. 
...
Through the use of geospatial data visualizations, the Relay Application is able to reveal high-level traffic patterns, providing invaluable insight into the workings of a city's transportation network. 
